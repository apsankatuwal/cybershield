...
function scan() {
  ...
  button.querySelector(".button-text").textContent = "Analyzing message...";

  button.querySelector(".scan-icon").textContent = "⏳";

  document.querySelector(".textarea-wrap").classList.add("scanning");
  ...
  setTimeout(() => {
    ...
    rules.forEach((rule) => {
      if (rule.words.some((word) => message.includes(word))) {
        score += rule.points;
        detected.push(rule);
      }
    });
    ...
    const prize = detected.some((item) => item.name === "Prize / Reward");

    const urgency = detected.some((item) => item.name === "Urgency / Pressure");
    ...
    if (prize && urgency) {
      ...
      detected.push({
        icon: "🚩",
        name: "Prize + Urgency",
        text: "An unexpected reward combined with time pressure is a strong scam indicator.",
      });
    }
    ...
    showResult(score, risk, icon, detected);

    document.querySelector(".textarea-wrap").classList.remove("scanning");

    button.disabled = false;

    button.querySelector(".scan-icon").textContent = "⌕";

    button.querySelector(".button-text").textContent = "Check this message";
  }, 1100);
}
...
function showResult(score, risk, icon, detected) {
  ...
  function animateScore(time) {
    const percentage = Math.min((time - start) / duration, 1);

    current = Math.floor(percentage * score);
...
function renderHistory() {
  ...
  const flagged = scanHistory.filter((entry) => entry.score >= 30).length;

  const avg = total
    ? Math.round(
        scanHistory.reduce((sum, entry) => sum + entry.score, 0) / total,
      )
    : 0;
...
function loadExample(type) {
  const examples = {
    ...
    bank: "URGENT! Your bank account will be suspended immediately. Verify your account now and enter your OTP.",

    safe: "Hey, are we still meeting at 5 PM today?",
  };
...
if (heroVisual) {
  document.addEventListener("mousemove", (event) => {
    ...
    heroVisual.style.marginLeft = `${x}px`;

    heroVisual.style.marginTop = `${y}px`;
  });
}
